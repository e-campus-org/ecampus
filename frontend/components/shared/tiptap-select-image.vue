<template>
    <v-alert class="mb-4" type="info" text="Select tab is custom component" />
    <v-select v-model="form.src" label="Select Image" :items="items" item-value="src" item-title="alt" />
    <v-text-field v-model="form.alt" label="Alt" />
    <v-checkbox v-model="form.lockAspectRatio" label="Lock original aspect ratio" />
</template>
