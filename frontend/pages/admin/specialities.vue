...
const addConfirm = async (item: object) => {
    try {
        ...
        const data = await useFetch<Specialities.ReadSpecialityDTO>('/specialities', {
            method: 'POST',
            body
        });
...
const deleteConfirm = async () => {
    try {
        if (specialitiesListData.value && deletedId.value) {
            await useFetch(`/specialities/${deletedId.value}`, { method: 'DELETE' })
...
const editConfirm = async (item: object) => {
    try {
        if (specialitiesListData.value && editedItem.value) {
            const data = await useFetch(`/specialities/${editedItem.value.id}`, {
                method: 'PUT',
                body: {
                    speciality: item
                }
